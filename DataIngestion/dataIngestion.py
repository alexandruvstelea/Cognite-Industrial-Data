    ...
    try:
        ...
        for asset in assets_list:
            if int(assets_list[f'{asset}'].get('assetID')) not in get_assets_in_db("all"):
                assets_to_add.append([assets_list[f'{asset}'].get('assetID'), assets_list[f'{asset}'].get(
                    'assetName'), assets_list[f'{asset}'].get('isUsed')])
...
def add_new_datapoints_to_db():
    ...
    for asset in assets_list:
        if int(assets_list[f'{asset}'].get('assetID')) in active_assets_id:
            assets_data.append(get_datapoints_list_format(int(assets_list[f'{asset}'].get(
                'datapointsID')), int(assets_list[f'{asset}'].get('assetID'))))
